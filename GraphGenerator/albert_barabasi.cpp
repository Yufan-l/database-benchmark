...
void parse_args(int argc, char **argv, int &n, float &m, int &c, float &p) {
  extern char *optarg;
  ...
  while ((copt = getopt(argc, argv, "n:m:c:p:h")) != EOF){
    switch (copt) {
    case 'h':       /* help */
...
int main(int argc, char **argv) {
  ...
  char file_name[100];
  sprintf(file_name, "AB_N_%d_M_%f",n,m);
  ofstream fout(file_name);
  
  fout<<"Node_Number "<<"\t"<<n<<endl;
